...
if (isset($_FILES["photo_field"]["name"]) && isset($_POST ['title_field']) && isset($_POST ['description_field']) && isset($_POST ['date_field']) && isset($_POST ['keywords_field'])) {
    ...
    // if all input fields are valid, proceed to uploading the photo
    if ($is_inputs_valid) {
        ...
        if (move_uploaded_file($_FILES["photo_field"]["tmp_name"], dirname(__FILE__).$target_dir.$file_name)) {
            ...
            try {
                ...
                $my_db = new MyDB('phpdb'); // Specify the database name "phpdb"
                ...
                $payload = "{\"bucketName\":\"tlai-photo-buckets\",\"fileName\":\"" . $file_name . "\"}";
                ...
                // redirect users to album.php once upload is completed
